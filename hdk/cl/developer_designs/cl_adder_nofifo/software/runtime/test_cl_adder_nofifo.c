...
#define PLUS_ONE_ADDR   UINT64_C(0x508)
#define TIMES_TWO_ADDR  UINT64_C(0x50C)
#define FIFO_ADDR   UINT64_C(0x510)
...
int peek_poke_example(uint32_t value, int slot_id, int pf_id, int bar_id) {
    ...
    rc = fpga_pci_peek(pci_bar_handle, FIFO_ADDR, &value);    //read back
	...
	//no fifo so that data has to be read immediately
